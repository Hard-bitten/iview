...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getOptionData(value){
                ...
                const search = (nodes)=>{
                    for(let j=0;j<nodes.length;j++){
                        ...
                        if(node.children){
                            let res = search(node.children);
                            if(res) return res;
                        }
                    }
                    return null;
                };
                const node = search(this.originData);
            ...
            checkUpdateStatus() {
                if (this.getInitialValue().length > 0 && this.originData.length === 0) {
                    this.hasExpectedValue = true;
                }
            },
            onOptionsChanges(options) {
                if (this.multiple){
                    ...
                    this.query = String(options[0].label).trim();
                    this.values = options;
                    this.lastRemoteQuery = '';
                    this.hideMenu();
                }
            ...
            setSelectNodes(){
                var checkeds = this.$refs.tree.getCheckedAndIndeterminateNodes();
                var tree = JSON.parse(JSON.stringify(this.originData));
                var selectPoint = [];
                const search = (node)=>{
                    if(node.checked){
                        selectPoint.push(node);
                        return;
                    }
                    if(node.children){
                ...
                this.onOptionsChanges(selectPoint); 
            },
            ...
            resetTree: function (callBack) {
                const vm = this;
                let cloneNode = JSON.parse(JSON.stringify(vm.originData.length > 0 ? vm.originData : vm.treeData));
                vm.traverseTree(
                {children: cloneNode},
                callBack
                );
